...
RUN cp -rvL _build/install/default ocamllsp-1.22.0-built-with-ocaml-5.3.1+relocatable-aarch64-linux-gnu
RUN tar czf ocamllsp-1.22.0-built-with-ocaml-5.3.1+relocatable-aarch64-linux-gnu.tar.gz ocamllsp-1.22.0-built-with-ocaml-5.3.1+relocatable-aarch64-linux-gnu

FROM scratch
COPY --from=builder /home/user/ocaml-lsp/ocamllsp-1.22.0-built-with-ocaml-5.3.1+relocatable-aarch64-linux-gnu.tar.gz .
